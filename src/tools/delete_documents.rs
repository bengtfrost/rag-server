...
pub async fn delete_documents(
    ...
    if !db_guard.collection_exists(&args.collection)? {
        return Err(anyhow::anyhow!("Fel: Samlingen '{}' finns inte.", args.collection));
    }

    if args.ids.is_empty() {
        let (doc_count, _) = db_guard.get_collection_stats(&args.collection)?;
        db_guard.clear_collection(&args.collection)?;
        return Ok(format!("✓ Samlingen '{}' är nu tom. {} dokument borttagna.", args.collection, doc_count));
    }
    ...
    let mut msg = format!("✓ Tog bort {} dokument från samlingen '{}'.", existing.len(), args.collection);
    if !missing.is_empty() {
        msg.push_str(&format!("\nVarning: Hittades inte och hoppades över: {}", missing.join(", ")));
    }
    Ok(msg)
}